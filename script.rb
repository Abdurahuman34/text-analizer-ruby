...
def read_document(file_name)
    File.read(file_name)  # define an argument and pass a parameter that Reads the content of the specified file and returns it as a string
end

# define an argument method to count the number of words in the document
def count_words(text)
    text.split(/\s+/).count  # Splits the text into words using whitespace as a delimiter and counts them
end

# define argument method to count the number of characters in the document
